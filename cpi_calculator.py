...
.ap-header {
  background:
    radial-gradient(120% 160% at 88% 12%, rgba(255,255,255,0.14) 0%, transparent 46%),
...
/* Faint diagonal sheen sweeping the header — depth cue from the reference. */
.ap-header::after {
...
/* Lit top edge — the reference's signature card treatment. Fades out at the
   corners so it reads as light falling on the panel, not a drawn border. */
.kpi::before {
  content:""; position:absolute; inset:0 0 auto 0; height:1px;
  background:linear-gradient(90deg, transparent, var(--hairline) 18%, var(--hairline) 82%, transparent);
  pointer-events:none;
}
.kpi:hover { transform:translateY(-2px); box-shadow:var(--shadow-md), inset 0 1px 0 var(--hairline); }
.kpi-label {
...
/* Hero card — primary decision metric. Wins on width + size + colour + glow. */
.kpi-hero {
  flex:1.8 1 200px; min-width:200px; border-left:4px solid var(--red-bright);
  background:
    radial-gradient(120% 140% at 100% 0%, color-mix(in srgb, var(--red-bright) 14%, transparent) 0%, transparent 60%),
    var(--card-grad);
  box-shadow:var(--glow-red), inset 0 1px 0 var(--hairline);
  padding:var(--s-3) var(--s-5);
}
.kpi-hero:hover { box-shadow:var(--glow-red), var(--shadow-md), inset 0 1px 0 var(--hairline); }
.kpi-hero .kpi-label { color:var(--red-bright); }
.kpi-hero .kpi-val { font-size:var(--num-lg); letter-spacing:-0.02em; }
...
/* Status wash tinting the panel from its accent edge — keeps the severity
   readable at a glance without repainting the whole card. */
.signal-box::before {
  content:""; position:absolute; inset:0; width:38%; pointer-events:none;
  background:linear-gradient(90deg, var(--sig-wash, transparent), transparent);
}
.signal-box > * { position:relative; z-index:1; }
.signal-box-red   { --sig-wash:color-mix(in srgb, var(--red-bright) 10%, transparent); }
.signal-box-amber { --sig-wash:color-mix(in srgb, var(--amber) 12%, transparent); }
.signal-box-green { --sig-wash:color-mix(in srgb, var(--green) 12%, transparent); }
...
.sec-title {
  ...
  padding-bottom:var(--s-2); margin-bottom:var(--s-3); margin-top:var(--s-5);
}
/* Short accent tick anchoring each section — replaces the heavy full underline. */
.sec-title::before {
...
.stDownloadButton > button {
  background:linear-gradient(135deg, #EB2D3C 0%, #C8102E 100%) !important;
  color:white !important;
  border:none !important; font-weight:600 !important;
  border-radius:var(--radius-sm) !important; padding:var(--s-2) var(--s-5) !important;
  font-size:var(--t-sm) !important; letter-spacing:0.01em !important;
...
/* Sticky header so long region/period tables keep their column context. */
.themed-table { max-height:520px; overflow:auto; }
.themed-table thead th {
  position:sticky; top:0; z-index:2;
  background:var(--card-2); color:var(--ink-600);
  font-size:0.68rem; font-weight:700; text-transform:uppercase; letter-spacing:0.05em;
  padding:11px 14px; border-bottom:1px solid var(--border); text-align:right; white-space:nowrap;
}
...
section[data-testid="stSidebar"] {
  ...
  box-shadow:1px 0 0 var(--ring), 4px 0 24px var(--scrim);
}
/* Sidebar section headers get the same accent tick as main section titles. */
section[data-testid="stSidebar"] .sec-title-sidebar {
  display:flex; align-items:center; gap:6px;
}
section[data-testid="stSidebar"] .sec-title-sidebar::before {
...
THEMES = {
    "Dark": {
        ...
        "shadow-sm": "0 1px 2px rgba(0,0,0,0.30), 0 1px 3px rgba(0,0,0,0.40)",
        "shadow-md": "0 2px 6px rgba(0,0,0,0.40), 0 8px 24px rgba(0,0,0,0.45)",
        # Depth system — reference-style elevated panels with a lit top edge.
        "card-grad": "linear-gradient(160deg, #1D1A28 0%, #17151F 55%, #141220 100%)",
        "hairline": "rgba(255,255,255,0.06)",   # inner top highlight on cards
        "glow-red": "0 0 0 1px rgba(255,90,115,0.22), 0 8px 28px rgba(255,90,115,0.14)",
        ...
        # Signal ramp — used by charts AND components so both themes stay in sync.
        "sig_high": "#FF5A73", "sig_mid": "#FBBF24", "sig_low": "#34D399",
        # Chart furniture (was hardcoded light-only greys)
        "chart_muted": "#4E4B5C",       # bar colour for non-selected regions
        "chart_baseline": "#4A4756",    # base=100 reference line
        "chart_halo": "#FFFFFF",        # selected-bar outline
        "chart_pill_border": "#3A3746",
        "chart_series": ["#FF5A73", "#60A5FA", "#34D399", "#FBBF24", "#A78BFA",
                         "#22D3EE", "#FB923C", "#F472B6", "#94A3B8"],
        "chip_sel_bg": "#FF5A73", "chip_sel_fg": "#1A0A0E",
        "chip_q_bg": "#60A5FA",   "chip_q_fg": "#0A1220",
    },
    "Light": {
        "red-bright": "#C8102E", "blue": "#1D4ED8", "amber": "#D97706", "green": "#16A34A",
        "ink-900": "#111827", "ink-700": "#374151", "ink-600": "#4B5563", "ink-400": "#9CA3AF",
        "border": "#E5E7EB", "surface": "#F4F5F7", "card": "#FFFFFF", "card-2": "#F3F4F6",
        "sidebar-bg": "#FAFAFA",
        "amber-ink": "#92400E", "blue-ink": "#1E40AF", "green-ink": "#14532D",
        "shadow-sm": "0 1px 2px rgba(16,24,40,0.04), 0 1px 3px rgba(16,24,40,0.06)",
        "shadow-md": "0 2px 4px rgba(16,24,40,0.06), 0 12px 28px rgba(16,24,40,0.10)",
        "card-grad": "linear-gradient(160deg, #FFFFFF 0%, #FFFFFF 55%, #FBFBFD 100%)",
        "hairline": "rgba(16,24,40,0.04)",
        "glow-red": "0 0 0 1px rgba(200,16,46,0.16), 0 8px 24px rgba(200,16,46,0.10)",
        "ring": "rgba(16,24,40,0.04)",
        "scrim": "rgba(16,24,40,0.10)",
        "monthly": "#C8102E", "quarterly": "#C8102E",
        "chart_bg": "#FFFFFF", "chart_grid": "#E9ECF1", "chart_font": "#111827", "chart_pill": "#FFFFFF",
        ...
        "sig_high": "#C8102E", "sig_mid": "#D97706", "sig_low": "#16A34A",
        "chart_muted": "#CBD5E1",
        "chart_baseline": "#94A3B8",
        "chart_halo": "#0F172A",
        "chart_pill_border": "#D8DEE7",
        "chart_series": ["#C8102E", "#1D4ED8", "#15803D", "#B45309", "#6D28D9",
                         "#0E7490", "#C2410C", "#BE185D", "#475569"],
        "chip_sel_bg": "#C8102E", "chip_sel_fg": "#FFFFFF",
        "chip_q_bg": "#1D4ED8",   "chip_q_fg": "#FFFFFF",
    },
}

# Active Python-side colour globals — reassigned by apply_theme() each run.
# NOTE: these initial values are placeholders only; apply_theme() runs first
# thing in main() and overwrites every one of them for the active theme.
MONTHLY_COLOR   = "#FF5A73"
QUARTERLY_COLOR = "#FF5A73"
CHART_BG   = "#15131C"
CHART_GRID = "#2A2733"
CHART_FONT = "#CFCDD9"
CHART_PILL = "#1F1C2B"
...
DOT_ND_BG = "#1A1723"; DOT_ND_FG = "#6E6B7E"; DOT_ND_BORDER = "#3A3746"
PILL_BG = "#1F1C2B"; PILL_BORDER = "#2B2836"; PILL_FG = "#CFCDD9"
# Signal ramp + chart furniture — theme-driven, never hardcode these at call sites.
SIG_HIGH = "#FF5A73"; SIG_MID = "#FBBF24"; SIG_LOW = "#34D399"
CHART_MUTED = "#4E4B5C"; CHART_BASELINE = "#4A4756"; CHART_HALO = "#FFFFFF"
CHART_PILL_BORDER = "#3A3746"
CHART_SERIES = THEMES["Dark"]["chart_series"]
CHIP_SEL_BG = "#FF5A73"; CHIP_SEL_FG = "#1A0A0E"
CHIP_Q_BG = "#60A5FA";   CHIP_Q_FG = "#0A1220"


def signal_color(v):
    """Map a percentage to the pricing-signal colour ramp (theme-aware).
    Thresholds mirror get_signal(): >=4 high, >=2 mid, else low."""
    v = abs(v or 0)
    return SIG_HIGH if v >= 4 else (SIG_MID if v >= 2 else SIG_LOW)
...
def apply_theme(theme_name: str):
    ...
    global SIG_HIGH, SIG_MID, SIG_LOW, CHART_MUTED, CHART_BASELINE, CHART_HALO
    global CHART_PILL_BORDER, CHART_SERIES, CHIP_SEL_BG, CHIP_SEL_FG, CHIP_Q_BG, CHIP_Q_FG
    ...
    SIG_HIGH, SIG_MID, SIG_LOW = t["sig_high"], t["sig_mid"], t["sig_low"]
    CHART_MUTED, CHART_BASELINE, CHART_HALO = t["chart_muted"], t["chart_baseline"], t["chart_halo"]
    CHART_PILL_BORDER, CHART_SERIES = t["chart_pill_border"], t["chart_series"]
    CHIP_SEL_BG, CHIP_SEL_FG = t["chip_sel_bg"], t["chip_sel_fg"]
    CHIP_Q_BG, CHIP_Q_FG = t["chip_q_bg"], t["chip_q_fg"]
    ...
    root_vars = ";".join(
        f"--{k}:{t[k]}" for k in (
            ...
            "card-grad", "hairline", "glow-red", "ring", "scrim",
        )
    )
...
def main():
    ...
    with st.sidebar:
        ...
        series_mode = st.radio(
            "Series mode",
            ["📅 Monthly   ·   ID : A130393720C", "📆 Quarterly   ·   ID : A2325846C"],
            index=0,
...
def _render_snapshot_month(df, period, frequency="Monthly", selected_city="Australia",
    ...
    yoy_signal_color = (MONTHLY_COLOR if (city_yoy or 0) >= 4
                        else (SIG_MID if (city_yoy or 0) >= 2 else SIG_LOW))
    ...
    idx_bar_colors = [series_color if c == selected_city else CHART_MUTED for c in sorted_cities]
    idx_line_colors = [CHART_HALO if c == selected_city else 'rgba(0,0,0,0)' for c in sorted_cities]
    ...
    fig.add_hline(y=100, line_dash="dash", line_color=CHART_BASELINE, line_width=1,
                  annotation=dict(text="Base Sep-2025 = 100", font=dict(size=8, color=CHART_FONT, weight="bold"),
                                  bgcolor=CHART_PILL, bordercolor=CHART_PILL_BORDER, borderwidth=1, borderpad=2),
    ...
    yoy_line_colors = [CHART_HALO if c == selected_city else 'rgba(0,0,0,0)' for c in sorted_cities]
    ...
    fig2.add_hline(y=2.0, line_dash="dot", line_color=SIG_MID, line_width=1,
                   annotation=dict(text="2% Monitor level", font=dict(color=SIG_MID, size=9),
                                   bgcolor=CHART_PILL, bordercolor=SIG_MID, borderwidth=1, borderpad=3),
                   annotation_position="bottom left")
    ...
    if frequency == "Monthly":
        ...
        for lbl in expected_labels:
            if lbl == period:
                dots += f'<span style="display:inline-block;background:{CHIP_SEL_BG};color:{CHIP_SEL_FG};border-radius:6px;padding:4px 10px;margin:3px;font-size:0.78rem;font-weight:700;">{lbl} ◀ selected</span>'
            elif lbl in available:
                is_qend = lbl.startswith(month_names[q_end_month])
                bg = CHIP_Q_BG if is_qend else DOT_BG
                fg = CHIP_Q_FG if is_qend else DOT_FG
...
def _render_yoy_evidence(df, city, period, frequency, default_expanded=True):
    ...
    if prior_yr_idx:
        ...
        rounds = (f'<br><span style="font-size:0.78rem;color:var(--ink-600);font-family:inherit;">→ ABS official: </span>'
                  f'<strong style="font-size:1.15rem;color:{MONTHLY_COLOR};">{curr_yoy:.1f}%</strong>') if curr_yoy is not None else ""
        yoy_block = f"""
        <div style="display:flex;gap:20px;align-items:flex-start;flex-wrap:wrap;">
          <div style="flex:0 0 auto;">
            <div class="ev-row">
              <span class="ev-step">①</span>
              <span class="ev-lbl">{period} &nbsp;<em style="color:var(--ink-400);">(current)</em></span>
              <span class="ev-val">{curr_f:.2f}</span>
            </div>
            <div class="ev-row">
              <span class="ev-step">②</span>
              <span class="ev-lbl">{prior_yr} &nbsp;<em style="color:var(--ink-400);">(one year ago)</em></span>
          ...
          <div class="ev-formula" style="flex:1;min-width:260px;margin-top:0;">
            ...
            &nbsp;&nbsp;&nbsp;&nbsp;= <strong style="color:{MONTHLY_COLOR};font-size:1.05rem;">{calc:+.2f}%</strong>{rounds}
          </div>
        </div>"""
    else:
        yoy_block = f"<p style='color:var(--ink-400);font-size:0.8rem;'>Prior year data ({prior_yr}) not yet available.</p>"
    ...
    with st.expander(f"📐  {city}  ·  {period}  —  Calculation Evidence", expanded=default_expanded):
        ...
        dot_colors = [spark_color if p == period else CHART_MUTED for p in spark_x]
        dot_sizes  = [10 if p == period else 6 for p in spark_x]
        fig_spark = go.Figure(go.Scatter(
            x=spark_x, y=spark_y,
            mode='lines+markers',
            line=dict(color=spark_color, width=2.5),
            marker=dict(color=dot_colors, size=dot_sizes, line=dict(color=CHART_HALO, width=1.5)),
            hovertemplate="<b>%{x}</b><br>Index: %{y:.2f}<extra></extra>",
        ))
...
def _render_single(df, city, start_period, end_period,
    ...
    with (chart_col1 if show_yoy_chart else st):
        ...
        fig = go.Figure()
        if show_all_regions:
            # Theme-driven categorical ramp, zipped to CITIES order.
            colors_map = dict(zip(CITIES, CHART_SERIES))
            for c in CITIES:
            ...
            fig.add_annotation(x=start_period, y=sv,
                ...
                bgcolor=CHART_PILL, bordercolor=CHART_PILL_BORDER, borderwidth=1,
        ...
        fig.add_hline(y=100, line_dash="dash", line_color=CHART_BASELINE, line_width=1,
                      annotation=dict(text="Base: Sep-2025 = 100",
                                      ...
                                      bgcolor=CHART_PILL, bordercolor=CHART_PILL_BORDER,
                                      borderwidth=1, borderpad=3),
                      annotation_position="top left")
        fig.update_layout(
        ...
        with chart_col2:
            ...
            if yoy_sub.empty:
                ...
                fig2.add_hline(y=4.0, line_dash="dash", line_color=MONTHLY_COLOR, line_width=1.5,
                               ...
                               annotation_position="top left")
                fig2.add_hline(y=2.0, line_dash="dot", line_color=SIG_MID, line_width=1,
                               annotation=dict(text="2% Monitor", font=dict(color=SIG_MID, size=9),
                                               bgcolor=CHART_PILL, bordercolor=SIG_MID,
                                               borderwidth=1, borderpad=3),
                               annotation_position="bottom left")
...
def _render_monthly_to_quarterly(df_monthly, city, start_period, end_period):
    ...
    # Monthly bars
    bar_colors = [
        MONTHLY_COLOR if row['Quarter'] == sub_m['Quarter'].iloc[-1] else CHART_MUTED
    ...
    if not sub_q.empty:
        fig.add_trace(go.Scatter(
            ...
            marker=dict(size=14, color=QUARTERLY_COLOR, symbol="diamond",
                        line=dict(color=CHART_HALO, width=2)),
    ...
    def style_row(row):
        if rollup_df.loc[row.name, 'Type'] == 'quarter':
            return [f'background-color:{PILL_BG}; font-weight:700; color:{MONTHLY_COLOR}'] * len(row)
        return [''] * len(row)
...
def _render_footer(df, frequency_label):
    ...
    st.markdown(f"""
    <div style="font-size:0.75rem;color:var(--ink-400);text-align:center;padding:6px 0;">
        Australia Post · CPI Calculator v3.1 · Data & Pricing Executive Tool ·
        Source: <a href="https://www.abs.gov.au/statistics/economy/price-indexes-and-inflation/consumer-price-index-australia/latest-release" target="_blank" style="color:var(--red-bright);">ABS 6401.0 ({frequency_label}, {latest_period})</a> ·
